    ...
    MCC_fnc_NameTagsEVH = {
        if ((missionNamespace getvariable ["MCC_nameTags",false]) || (missionNamespace getvariable ["MCC_medicShowWounded",false])) then {
           ...
           if (player != vehicle player && (missionNamespace getvariable ["MCC_nameTags",false])) then {
                ...
                _driver         = if (alive (driver vehicle player)) then {name (driver vehicle player)} else {"Нет"};
                _gunner         = if (alive (gunner vehicle player)) then {name (gunner vehicle player)} else {"Нет"};
                _commander      = if (alive (commander vehicle player)) then {name (commander vehicle player)} else {"Нет"};
                ...
                _string = _string + format ["<t font='puristaMedium' size='0.4' align='left' color='#a8e748'>Командир: %1 </t><br/>",_commander];
                _string = _string + format ["<t font='puristaMedium' size='0.4' align='left' color='#a8e748'>Водитель: %1 </t><br/>",_driver];
                _string = _string + format ["<t font='puristaMedium' size='0.4' align='left' color='#a8e748'>Стрелок: %1 </t><br/>",_gunner];
                _string = _string + format ["<t font='puristaMedium' size='0.4' align='left' color='#a8e748'>Пустые места: %1 </t><br/>",_emptyPos];
