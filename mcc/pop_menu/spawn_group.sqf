...
{
	if (count mcc_zone_pos == 0) exitWith {hint "Создайте Зону"};	//Failsafe incase we trying to spawn something without making a zone first
	_zonePos = (mcc_zone_pos select (lbCurSel MCCZONENUMBER)+1);
	if (isnil "_zonePos") exitWith {hint "Создайте Зону"};	//Failsafe incase we trying to spawn something without making a zone first
	//Group
