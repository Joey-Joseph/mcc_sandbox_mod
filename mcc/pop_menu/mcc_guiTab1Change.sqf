...
if ((_task ==1)&& (MCC_type_index != lbCurSel SPAWNTYPE)) then																	//Change Type
{
	...
	if ((lbCurSel SPAWNTYPE) == 0) then {						//Singles
			...
			lbClear _comboBox;
			{
				_displayname =  _x;
				_index = _comboBox lbAdd _displayname;
			} foreach ["Пехота", "Транспорт", "Бронетехника/Статичные", "Мотоциклы", "Вертолеты", "Самолеты", "Лодки", "D.O.C", "Патроны", "Укрепления", "Могилы", "Мебель", 
						"Рынок", "Разное", "Знаки", "Город", "Обломки", "Здания", "Руины","Мусор","Лампы","Контейнеры","Маленькие предметы","Структуры","Помощники","Тренировка"];
