...
switch (true) do {
	...
	case (_ctrlData == "radio"): {
		_array = [
				   ["[(missionNamespace getVariable ['MCC_interactionLayer_0',[]]),1] spawn MCC_fnc_interactionsBuildInteractionUI","Назад",format ["%1mcc\interaction\data\iconBack.paa",MCC_path]],
				   ["setCurrentChannel 3","Группа","\a3\Ui_F_Curator\Data\Displays\RscDisplayCurator\modeGroups_ca.paa"],
				   ["setCurrentChannel 5","Прямой",format ["%1data\iconOrder.paa",MCC_path]]
				 ];
		...
		if (player == leader player && ((channelEnabled 2) select 0 || (channelEnabled 2) select 1)) then {
			_array pushback (["setCurrentChannel 2","Командный","\A3\Ui_f\data\GUI\Cfg\Ranks\sergeant_gs.paa"]);
		};
		...
		if (getPlayerUID  player == (MCC_server getVariable [format ["CP_commander%1",(player getVariable ["CP_side",  playerside])],""]) && ((channelEnabled 1) select 0 || (channelEnabled 1) select 1)) then {
			_array pushback (["setCurrentChannel 1","Дополнительный","\A3\Ui_f\data\GUI\Cfg\Ranks\colonel_gs.paa"]);
		};

		//Global
		if (serverCommandAvailable "#kick" || isServer) then {
			_array pushback (["setCurrentChannel 0","Глобальный","\A3\Ui_f\data\GUI\Cfg\Ranks\general_gs.paa"]);
		};
