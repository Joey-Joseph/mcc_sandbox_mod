...
if (tolower _planeType in ["west","east","guer","civ","logic"]) then  {
	...
	if (tolower _spawnkind == "ac-130") exitWith {
		...
		if (alive(missionNamespace getVariable ["MCC_ACConsoleUp",objNull])) then {
			...
			[[2,compile format ['["MCCNotifications",["AC-130 покинул зону","%1data\AC130_icon.paa",""]] call bis_fnc_showNotification;',MCC_path]], "MCC_fnc_globalExecute", playerSide, false] spawn BIS_fnc_MP;
		} else {

			[[2,compile format ['["MCCNotifications",["AC-130 вошел в зону","%1data\AC130_icon.paa",""]] call bis_fnc_showNotification;',MCC_path]], "MCC_fnc_globalExecute", playerSide, false] spawn BIS_fnc_MP;
	...
	if (_spawnkind in ["Авиаподдержка - (Пушка)","Авиаподдержка - (Ракеты)","Авиаподдержка - (П+Р)","Авиаподдержка - (Бомба)"]) then {


		private _casType =   switch (_spawnkind) do
								{
									case "Авиаподдержка - (Пушка)": {0};
									case "Авиаподдержка - (Ракеты)":{1};
									case "Авиаподдержка - (П+Р)":{2};
									case "Авиаподдержка - (Бомба)":{3};
		};
		...
		if (_spawnkind == "Найти и уничтожить") then	{
		...
		{
			case "Найти и уничтожить":	//Seek and Destroy
			{
			...
			case "Обстрел ракетами":	//Rockets run
			{
				waitUntil {((_plane1 distance2D _pos) < 500) || (!alive _plane1)};
				//Make the drop
				if (!alive _plane1) exitWith {};
			...
			case "Уничтожить технику":	//SnD Armor
			{
			...
			case "Уничтожить воздух":	//SnD Air
			{
