...
if (_action == 2) exitWith
{
	...
	lbClear _comboBox;
	{
		_displayname = _x;
		_comboBox lbAdd _displayname;
	} foreach ["Нет","Телепорт","Парашут","HALO"];
...
if (_action == 5) exitWith
{
	...
	lbClear _comboBox;
	{
		_displayname = _x;
		_index = _comboBox lbAdd _displayname;
	} foreach ["0","20 сек","40 сек","1 мин","80  сек","100  сек","2 мин","140 сек","160 сек","3 мин"];
	_comboBox lbSetCurSel 0;
};
...
if (_action == 6) exitWith
{
	...
	lbClear _comboBox;
	{
		_displayname =  _x;
		_index = _comboBox lbAdd _displayname;
	} foreach ["Пехота", "Транспорт", "Бронетехника", "Мотоциклы", "Вертолеты", "Самолеты", "Лодки", "Патроны"];
...
if (_action == 7) exitWith
{
	...
	lbClear _comboBox;
	{
		_displayname = _x;
		_index = _comboBox lbAdd _displayname;
	} foreach ["Вертолеты", "Транспорт","Бронетехника", "Лодки"];
	_comboBox lbSetCurSel 0;
	...
	if (count _evacVehicles > (missionNameSpace getvariable ["MCC_evacVehicles_index",0])-1 && ((missionNameSpace getvariable ["MCC_evacVehicles_index",0]) >= 0)) then {
		...
		_insetionArray = ["Движение (двиг вкл)","Движение (двиг выкл)"];
		...
		{
			_insetionArray = ["Посадка (Двиг вкл)","Посадка (Двиг выкл)","Зависнуть","Зависнуть (над водой)","По дыму","По веревке","Точная посадка"];
...
if (_action == 8) exitWith
{
	...
	lbClear _comboBox;
	{
		_displayname = _x;
		_index = _comboBox lbAdd _displayname;
	} foreach ["Маленькие объекты", "Средние объекты", "Обломки", "Спрятанное СВУ", "Минное поле", "Патроны", "Транспорт", "Вооруженные гражданские","Моджахеды"];
	_comboBox lbSetCurSel 0;
	...
	lbClear _comboBox;
	{
		_displayname = _x;
		_index = _comboBox lbAdd _displayname;
	} foreach ["Маленький", "Средний", "Большой"];
	_comboBox lbSetCurSel 0;
	...
	lbClear _comboBox;
	{
		_displayname = _x;
		_index = _comboBox lbAdd _displayname;
	} foreach ["Убивает", "Отключает", "Фейк", "Нет"];
	_comboBox lbSetCurSel 0;
	...
	lbClear _comboBox;
	{
		_displayname = _x;
		_index = _comboBox lbAdd _displayname;
	} foreach ["10 Сек", "20 Сек", "30 Сек", "40 Сек", "50 Сек", "1 Мин", "2 Мин", "3 Мин", "4 Мин", "5 Мин"];
	_comboBox lbSetCurSel 0;
	...
	lbClear _comboBox;
	{
		_displayname = _x;
		_index = _comboBox lbAdd _displayname;
	} foreach ["Вблизи", "Радио - Разведчик", "Мастер Игры","Мини-игра(Вблизи)","Мини-игра(Вручную)"];
	_comboBox lbSetCurSel 0;
	...
	_index = _comboBox lbAdd "Разведчик - Гражданский";
	_index = _comboBox lbAdd "Разведчик - Камуфляж";
	_comboBox lbSetCurSel 0;
};
//-------------------------------------------------------------------------------------CONVOY----------------------------------------------------------------------------------------------
...
if (_action == 16) exitWith
{
	...
	lbClear _comboBox;
	{
		_displayname = _x;
		_comboBox lbAdd _displayname;
	} foreach ["Транспорт", "Бронетехника", "Мотоциклы", "Лодки", "Патроны"];
	_comboBox lbSetCurSel 0;
...
if (_action == 17) exitWith
{
	...
	lbClear _comboBox;
	{
		...
		_comboBox lbAdd _displayname;
			  ...
			  "Запереть двери (Все)",
			  "Опереть двери(Все)",
			  "Запереть двери (Рандом)",
			  "Атмосфера - Боевая зона",
			  "Атмосфера - Песочная буря",
			  "Атмосфера - Шторм",
			  "Атмосфера - Снег",
			  "Атмосфера - Тепловой удар",
			  "Атмосфера - Убрать",
			  "Удалить ПНВ у юнитов",
			  "Добавить фонарики юнитам",
			  "Удалить трупы"];
	_comboBox lbSetCurSel 0;
};
