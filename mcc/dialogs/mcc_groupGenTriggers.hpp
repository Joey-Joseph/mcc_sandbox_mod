...
class MCC_triggersDialogControls: MCC_RscControlsGroup
{
	...
	{			
		...
		{
			idc = -1;
			text = "Генератор Триггеров:"; //--- ToDo: Localize;
		...
		{
			idc = -1;
			text = "Доступные триггеры:"; //--- ToDo: Localize;
			x = 0.00572965 * safezoneW;
		...
		class MCC_triggerGenCreate: MCC_RscButton
		{
			...
			tooltip = "Создать триггер"; //--- ToDo: Localize;
			sizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 0.8)";
		};
		class MCC_triggerGenMove: MCC_RscButton
		{
			idc = -1;
			text = "Двигать"; //--- ToDo: Localize;
