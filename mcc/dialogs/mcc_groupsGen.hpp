...
class mcc_groupGen
{
	...
	//---------------------------------------- BENCHMARK ----------------------------------------
		...
		class MCC_cacheButton: MCC_RscButton
		{
			...
			text = "Закэшировать группу"; //--- ToDo: Localize;
			sizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 0.7)";
		...
		{
			idc = -1;
			onButtonClick = "if (str (getmarkerpos str mcc_active_zone) != str [0,0,0]) then {[str mcc_active_zone] spawn GAIA_fnc_occupy} else {systemchat 'Создайте Зону'}";
			tooltip = "Occupy current selected zone with the selected faction's units";
			text = "Оккупировать зону"; //--- ToDo: Localize;
			sizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 0.7)";
		...
		{
			idc = 1015;
			onButtonClick = "_ctrl = _this select 0; if (ctrlText _ctrl == 'Динам. Бои(вкл)') then {_ctrl ctrlSetText 'Динам. Бои(откл)';MCC_GAIA_AC = false;} else {_ctrl ctrlSetText 'Динам. Бои(вкл)';MCC_GAIA_AC = true}; publicVariable 'MCC_GAIA_AC';";
			tooltip = "Spawn random ambient battlefield around the player";
			text = "Динам. Бои(откл)"; //--- ToDo: Localize;
			sizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 0.7)";
		...
		class MCC_MissionWIn: MCC_RscButton
		{
			...
			text = "Задание Выполненно"; //--- ToDo: Localize;
			colorText[] = {0,1,0,0.8};
		...
		class MCC_MissionLost: MCC_RscButton
		{
			...
			text = "Задание Провалено"; //--- ToDo: Localize;
			colorText[] = {1,0,0,0.8};
