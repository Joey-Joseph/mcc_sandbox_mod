...
//set ruler data
ctrlSetText [MCC_ConsoleMapRulerDir,format ["Направление: %1",MCC_ConsoleRulerData select 0]];
ctrlSetText [MCC_ConsoleMapRulerDis,format ["Дистанция: %1м",MCC_ConsoleRulerData select 1]];
...
if (count _evacVehicles > 0) then {
	...
	if (_type iskindof "helicopter") then {									//Case we choose aircrft
		...
		_insetionArray = ["Посадка (Двиг вкл)","Посадка (Двиг выкл)","Зависнуть","Зависнуть (над водой)","По дыму","По веревке"];
