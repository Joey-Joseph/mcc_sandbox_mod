...
while {_time > time && dialog && (isNil "MCC_fakeAC")} do {
	_message = "";
	_messageFinal = ["П","о","д","к","л","ю","ч","е","н","и","е"," ","к"," ","A","C","-","1","3","0"];
			...
			while {MCC_Console3Open && dialog && alive _uav} do {
				...
				_data =
					[
						...
						format ["Осталось: %1",[MCC_ConsoleACTime -(time - (missionNameSpace getVariable ["MCC_ConsoleACTimeStart",0]))] call MCC_fnc_time],
				...
				ctrlSetText [MCC_CONSOLE_AC_DIR_TEXT, format ["Направление: %1",floor (getdir MCC_fakeAC)]];
