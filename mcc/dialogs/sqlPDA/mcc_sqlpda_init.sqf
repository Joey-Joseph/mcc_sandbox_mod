...
MCC_fnc_SQLPDAMenuclicked =
{
	...
	switch (true) do
	{
		...
		case (_ctrlData in ["enemy"]):
		{
			...
			_array = [
					   ["inf","Пехота",format["\A3\ui_f\data\map\markers\nato\%1inf.paa",_path]],
					   ["mech_inf","Мотопехота",format["\A3\ui_f\data\map\markers\nato\%1motor_inf.paa",_path]],
					   ["motor_inf","Механизированная пехота",format["\A3\ui_f\data\map\markers\nato\%1mech_inf.paa",_path]],
					   ["armor","Бронетехника",format["\A3\ui_f\data\map\markers\nato\%1armor.paa",_path]],
					   ["air","Вертолет",format["\A3\ui_f\data\map\markers\nato\%1air.paa",_path]],
					   ["plane","Самолет",format["\A3\ui_f\data\map\markers\nato\%1plane.paa",_path]],
					   ["naval","Лодка",format["\A3\ui_f\data\map\markers\nato\%1naval.paa",_path]],
					   ["art","Артиллерия",format["\A3\ui_f\data\map\markers\nato\%1art.paa",_path]],
					   ["installation","Сооружение",format["\A3\ui_f\data\map\markers\nato\%1Installation.paa",_path]],
					   ["unknown","Неизвестное",format["\A3\ui_f\data\map\markers\nato\%1unknown.paa",_path]],
					   ["MinefieldAP","Минное поле","\a3\Ui_F_Curator\Data\CfgMarkers\minefieldAP_ca.paa"]
					 ];
		...
		case (_ctrlData in ["inf","mech_inf","motor_inf","armor","air","plane","naval","art"]):
		{
			...
			_array = [
					   ["Огневая группа","Огневая группа",format["%1_0.paa",_path]],
					   ["Отряд","Отряд",format["%1_1.paa",_path]],
					   ["Группа","Группа",format["%1_2.paa",_path]],
					   ["Взвод","Взвод",format["%1_3.paa",_path]],
					   ["Батальон","Батальон",format["%1_4.paa",_path]]
					 ];
		};
		...
		case (_ctrlData in ["Огневая группа","Отряд","Группа","Взвод","Батальон"]):
		{
			_array = [];
...
//------------------------------------------------------------------------------------Menu - support----------------------------------------------------------------------------------------------------------------------
		case (_ctrlData in ["support"]):
		{
			...
			_array = [
					   ["mil_destroy","Нужна поддержка с воздуха",format["%1destroy_CA.paa",_path]],
					   ["mil_pickup","Нужен транспорт",format["%1pickup_CA.paa",_path]],
					   ["mil_objective","Нужна атака по территории",format["%1objective_CA.paa",_path]],
					   ["mil_marker","Нужна поддежка",format["%1marker_CA.paa",_path]],
					   ["mil_warning","Нужен медик",format["%1warning_CA.paa",_path]],
					   ["mil_warning","Нужны патроны",format["%1warning_CA.paa",_path]],
					   ["mil_warning","Нужен ремонт",format["%1warning_CA.paa",_path]],
					   ["mil_warning","Нужно топливо",format["%1warning_CA.paa",_path]]
					 ];
		...
		{
			_array = [];
			_text = ["Нужна поддержка с воздуха","Нужен транспорт","Нужна атака по территории","Нужна поддежка","Нужен медик","Нужны патроны","Нужен ремонт","Нужно топливо"] select _index;
			//clear
			[] call MCC_fnc_SQLPDAMenuClear;
...
//------------------------------------------------------------------------------------Menu - Construct----------------------------------------------------------------------------------------------------------------------
		case (_ctrlData in ["build"]):
		{
			...
			_path = "\A3\ui_f\data\map\markers\military\";
			_array = [
					   ["fob","Аванпост","\A3\ui_f\data\map\mapcontrol\Bunker_CA.paa"],
					   ["bunker","Бункер","\A3\ui_f\data\map\mapcontrol\Stack_CA.paa"],
					   ["hmg","Пулемет","\A3\Static_f_gamma\data\ui\gear_StaticTurret_MG_CA.paa"],
					   ["hmgh","Пулемет(установленный)","\A3\Static_f_gamma\data\ui\gear_staticturret_mg_high_ca.paa"],
					   ["gmg","Гранатомет","\A3\Static_f_gamma\data\ui\gear_StaticTurret_GMG_CA.paa"],
					   ["gmgh","Гранатомет(установленный)","\A3\Static_f_gamma\data\ui\gear_staticturret_gmg_high_ca.paa"],
					   ["at","ПТ установка","\A3\Static_F_Gamma\data\UI\gear_StaticTurret_AT_CA.paa"],
					   ["aa","ПВО установка","\A3\Static_F_Gamma\data\UI\gear_StaticTurret_AA_CA.paa"],
					   ["mortar","Миномет","\A3\Static_f\Mortar_01\data\UI\Mortar_01_ca.paa"]
					 ];
