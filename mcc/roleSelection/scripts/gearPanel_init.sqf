...
0 spawn {
	...
	while {dialog && (missionNamespace getVariable ["CP_gearPanelOpen",false])} do {
		...
		if (_oldCommander != _commander) then {
			...
			if (_commander == "") then {
				(_disp displayCtrl 919192) ctrlSetText "Взять";
			} else {
				if (_commander == getPlayerUID player) then	{
					(_disp displayCtrl 919192) ctrlSetText "Выйти";
				} else {
					(_disp displayCtrl 919192) ctrlSetText "Мятеж";
				};
			};
		...
		for "_i" from 0 to (count _cfg -1) do {
			...
			//Update levels
			(_disp displayCtrl (_idc + 400)) ctrlSetStructuredText parseText format ["<t color='#ffffff' size='0.8' shadow='1' align='cent'>Уровень: %1</t><br/><br/>", call compile format ["%1Level select 0",toLower _cfgName]];
