...
//Become a commander
if (_commander == "") then {
    _str = "<t size='1' font = 'puristaLight' color='#FFFFFF'>" + format ["%1 стал командиром",name player] + "</t>";
    ...
    if (_commander == getPlayerUID player) then {
        _str = "<t size='1' font = 'puristaLight' color='#FFFFFF'>" + format ["%1 теперь больше не командир",name player] + "</t>";
        _command = format ['["MCC_woosh",true] spawn BIS_fnc_playSound; ["%1",0,0.2,5,1,0.0] spawn bis_fnc_dynamictext;',_str];
        [[2,compile _command], "MCC_fnc_globalExecute", true, false] spawn BIS_fnc_MP;
