...
if (isnil "CP_defaultGroups") then {CP_defaultGroups = ["Альфа","Браво","Чарли","Дельта"]}; 	//Default squads names
...
//------------------------Artillery---------------------------------------------------
MCC_artilleryTypeArray = [["DPICM","GrenadeHand",0,40],["HE 120мм","Sh_120mm_HE_Tracer_Red",1,30], ["HE 155mm","Sh_155mm_AMOS",1,120], ["Кластер ПП","Mo_cluster_AP",3,32],["Мины 120мм","Mine_155mm_AMOS_range",3,120],["HE Лазерное наведение","Bo_GBU12_LGB",3,50],["HE 82мм","Sh_82mm_AMOS",1,75], ["Зажигательная 82мм","Fire_82mm_AMOS",1,35],["Белый дым 120мм","Smoke_120mm_AMOS_White",1,20],["Белый дым 82мм","Smoke_82mm_AMOS_White",1,20],["Зеленый дым 40мм","G_40mm_SmokeGreen",1,20], ["Красный дым 40мм","G_40mm_SmokeRed",1,20],["Сигнальная белая","F_40mm_White",2,20], ["Сигнальная зеленая","F_40mm_Green",2,20], ["Сигнальная красная","F_40mm_Red",2,20]];
MCC_artillerySpreadArray = [["Нет",0], ["Мальнький",100], ["Средний",200], ["Большой",400]]; //Name and spread in meters
...
//----------------------------Presets---------------------------------------------------------
mccPresetsVehicle = [
					 ['Убрать Топливо', '_this setfuel 0;']
					,['Убрать Патроны', '_this setvehicleammo 0;']
					,['Удалить Груз', 'clearMagazineCargoGlobal _this; clearWeaponCargoGlobal _this; clearItemCargoGlobal _this;']
					,['Закрыть технику', '_this setVehicleLock "LOCKED";']
					,['Удалить Груз (для одиночной)', 'clearMagazineCargo _this; clearWeaponCargo _this; clearItemCargo _this;']
					,['Добавить ИИ для БПЛА','if (isServer) then {createVehicleCrew _this;group _this setvariable ["MCC_canbecontrolled",true,true];};']
					,['Добавить пассажиров', 'if (isServer) then {[_this] call MCC_fnc_populateVehicle};']
					,['ECM - глушилка для IED','if (isServer) then {_this setvariable ["MCC_ECM",true,true]};']
					,['Мобильный штаб','_this addAction ["<t color=""#99FF00"">Создать мобильный штаб </t>", "'+MCC_path+'mcc\roleSelection\scripts\createFOB.sqf",[],6,false, false,"teamSwitch","(driver vehicle _target == _this) && (speed (vehicle _target) == 0)"];']
					,['Выключить симуляцию','_this enableSimulation false;']
					,['Может управляться через КОМЛОГ', '(group _this) setvariable ["MCC_canbecontrolled",true,true];']
					,['Нанять бота', '_this addAction [format ["Нанять %1", name _this], "'+MCC_path+'mcc\general_scripts\hostages\hostage.sqf",[2],6,false,true];']
					,['Присоединиться к игроку', '[_this] join (group player);']
					,['Сделать противником', '_this addrating -2001;']
					,['Мобильный респавн(Синие)','[west,_this] call BIS_fnc_addRespawnPosition;']
					,['Мобильный респавн(Красные)','[east,_this] call BIS_fnc_addRespawnPosition;']
					,['Мобильный респавн(Зеленые)','[resistance,_this] call BIS_fnc_addRespawnPosition;']
					,['', '']
					,['======= Артилерия =======','']
					,['Динамическая артилерия - Пушка', '[0,_this] spawn MCC_fnc_amb_Art;']
					,['Динамическая артилерия - Ракеты', '[1,_this] spawn MCC_fnc_amb_Art;']
					,['Динамическое ПВО - Пушка/Ракеты', '[2,_this] spawn MCC_fnc_amb_Art;']
					,['Динамическое ПВО - Прожектор', '[3,_this] spawn MCC_fnc_amb_Art;']
					,['======= Главное =======','']
					,['Уничтожить транспорт', '_this setdamage 1;']
					,['Перевернуть транспорт', '[_this ,0, 90] call bis_fnc_setpitchbank;']
					,['Вертуальный арсенал (BIS)', 'if (isServer) then {["AmmoboxInit",[_this,true]] call BIS_fnc_arsenal};']
					,['Уничтожается взрыв-пакетом', '_this addEventHandler ["handledamage", {if ((_this select 4) in ["SatchelCharge_Remote_Ammo","DemoCharge_Remote_Ammo"]) then {(_this select 0) setdamage 1;(_this select 3) addRating 1500} else {0}}];']
					,['Неуязвимость', '_this allowDamage false;']
					,['Консоль Корабля', '[_this,"Открыть Консоль Корабля","","","(alive _target) && (_target distance _this < 5)","(alive _target) && (_target distance _this < 5)",{},{},{[] spawn MCC_fnc_LHDspawnMenuInit},{},[],3,10,false,false] call bis_fnc_holdActionAdd;']
					,['', '']
					,['======= Эффекты =======','']
					,['Песочная буря','[_this] call BIS_fnc_sandstorm;']
					,['Мухи','[getposatl _this] call BIS_fnc_flies;']
					,['Дым','if (isServer) then {_effect = "test_EmptyObjectForSmoke" createVehicle (getpos _this);_effect setpos (getpos _this)};']
					,['Огонь','if (isServer) then {_effect = "test_EmptyObjectForFireBig" createVehicle (getpos _this);_effect setpos (getpos _this)};']
					,['', '']
					,['======= Разное =======','']
					,['Создать локальный маркер', '_this execVM "'+MCC_path+'mcc\general_scripts\create_local_marker.sqf";']
					];

mccPresetsUnits = [
					 ['Нанять бота', '_this addAction [format ["Нанять %1", name _this], "'+MCC_path+'mcc\general_scripts\hostages\hostage.sqf",[2],6,false,true];']
					,['Сделать заложником', '_this execVM "'+MCC_path+'mcc\general_scripts\hostages\create_hostage.sqf";']
					,['Присоединиться к игроку', '[_this] join (group player);']
					,['Сделать проивником', '_this addrating -2001;']
					,['Встать', '_this setUnitPos "UP";']
					,['Присесть', '_this setUnitPos "Middle";']
					,['Лечь', '_this setUnitPos "DOWN";']
					,['Удалить все оружие', 'removeAllWeapons _this;']
					,['Удалить все вещи', 'removeAllItems _this;']
					,['Без сознания (MCC мед. система)', '[_this,_this] spawn MCC_fnc_unconscious']
					,['Может управляться через КОМЛОГ', '(group _this) setvariable ["MCC_canbecontrolled",true,true];']
					,['Неуязвимость', '_this allowDamage false;']
					,['', '']
					,['======= Главное =======','']
					,['Уничтожить юнита', '_this setdamage 1;']
					,['Повернуть юнита', '[_this ,0, 90] call bis_fnc_setpitchbank;']
					,['Уничтожается взрыв-пакетом', '_this addEventHandler ["handledamage", {if ((_this select 4) in ["SatchelCharge_Remote_Ammo","DemoCharge_Remote_Ammo"]) then {(_this select 0) setdamage 1;(_this select 3) addRating 1500} else {0}}];']
					,['', '']
					,['======= Эффекты =======','']
					,['Песочная буря','[_this] call BIS_fnc_sandstorm;']
...
mccPresetsObjects = [
					 ['Можно подобрать','_this call MCC_fnc_pickItem;']
					,['Выключить симуляцию','_this enableSimulation false;']
					,['Уничтожить объект', '_this setdamage 1;']
					,['Перевернуть объект', '[_this ,0, 90] call bis_fnc_setpitchbank;']
					,['Виртуальный арсенал (BIS)', '["AmmoboxInit",[_this,true]] call BIS_fnc_arsenal']
					,['Уничтожается взрыв-пакетом', '_this addEventHandler ["handledamage", {if ((_this select 4) in ["SatchelCharge_Remote_Ammo","DemoCharge_Remote_Ammo"]) then {(_this select 0) setdamage 1;(_this select 3) addRating 1500} else {0}}];']
					,['Неуязвимость', '_this allowDamage false;']
					,['Консоль Корабля', '[_this,"Открыть Консоль Корабля","","","(alive _target) && (_target distance _this < 5)","(alive _target) && (_target distance _this < 5)",{},{},{[] spawn MCC_fnc_LHDspawnMenuInit},{},[],3,10,false,false] call bis_fnc_holdActionAdd;']
					,['', '']
...
MCC_ZoneLocation = [["Сервер", 0], ["Удаленный клиент", 1]]; //NEW
...
MCC_musicActivateby_array = ["Нет","Красные","Синие","Зеленые","Гражданские","Логика","Любые","Альфа","Браво","Чарли","Дельта","Эхо","Фокстрот","Гольф","Хоутел","Индия","Джульет","Статичное","Транспорт","Группа","Лидер","Участник","Захвачено Синими","Захвачено Красными","Захвачено Зелеными"];
MCC_musicCond_array = ["Существует","Не существует","Синие D","Красные D","Зеленые D","Гражданские D"];
//MCC_angle_array = [0,45,90,135,180,225,270,315]; // no longer used
MCC_shapeMarker = ["RECTANGLE","ELLIPSE"];
MCC_colorsarray = [["Черный","ColorBlack"],["Белый","ColorWhite"],["Красный","ColorRed"],["Зеленый","ColorGreen"],["Синий","ColorBlue"],["Желтый","ColorYellow"]];

MCC_spawn_empty =[["Нет",true],["Да",false]];
mcc_spawnbehavior = "";
MCC_spawn_behaviors = [
                      ["Агрессивный", "MOVE","ИИ будет патрулировать зону и будет высылать подкрепления вне зоны"],
					  ["Оборонительный","NOFOLLOW","ИИ будет патрулировать зону, но на контакт вне зоны вступать не будет"],
					  ["Укрепления","FORTIFY","ИИ будет искать статичные орудия и пустые здания и занимать их"],
					  ["Нет","bis","Обычное поведение ИИ"],
					  ["BIS Оборона","bisd","ИИ будет держать оборону и патрулировать"],
					  ["BIS Патруль","bisp","ИИ будет патрулировать"]
					  ];
...
MCC_spawn_awereness = [["Стандартно", "default"],["Начеку","Aware"],["В бою", "Combat"],["Скрытно","stealth"],["Безразлично","Careless"]];
MCC_empty_index = 0;
...
MCC_months_array = [["Январь", 1],["Февраль",2],["Март", 3],["Апрель",4],["Май",5],["Июнь", 6],["Июль",7],["Август", 8],["Сентябрь",9],["Октябрь",10],["Ноябрь",11],["Декабрь",12]];
...
MCC_weather_array = [["Ясно",[0, 0, 0, 0, 0]], ["Облачно",[0.5, 0.5, 0.5, 0.5, 0.5]],["Дождь",[0.8, 0.8, 0.8, 0.8, 0.8]],["Гроза",[1, 1, 1,1,1]]];

MCC_grass_array = [["Без травы",50],["Низкая",25], ["Средняя",12.5], ["Высокая",6.25], ["Очень высокая",3.125]];
...
MCC_deleteTypes = ["Люди", "Машины", "Бронетехника", "Вертолеты", "Самолеты", "Патроны", "Все"];
...
MCC_CASBombs = ["Авиаподдержка - (Пушка)","Авиаподдержка - (Ракеты)","Авиаподдержка - (П+Р)","Авиаподдержка - (Бомба)","Найти и уничтожить","Обстрел ракетами","Уничтожить технику","Уничтожить воздух","Умная бомба","Лазерное наведение","Бомбежка","Cruise Missile","AC-130","UAV","Controllable"]; //
...
MCC_evacFlyInHight_array = [["20м",20],["50м",50],["100м",100],["150м",150],["200м",200],["300м",300],["400м",400],["500м",500]];
...
MCC_groupFormation	= ["Колонна","Шахматный","Клин","Лев Пеленг","Прав Пеленг","Обр Клин","Линия","Ряд","Ромб"];	//Group formations
...
if ( isServer ) then {
	...
	*/
	...
	MCC_missionMakerDC =
	{
		...
		{
			_str = "<t size='1' font = 'puristaLight' color='#FFFFFF'>" + format ["%1 больше не командир",_name] + "</t>";
...
if (isPlayer player && !isServer && !(MCC_isLocalHC) && (missionNameSpace getVariable ["MCC_syncOn",true])) then {
	0 spawn {
		...
		for [{_x=1},{_x<=_loop},{_x=_x+1}]  do {
			...
			//add header
			_html = "<t color='#818960' size='1.2' shadow='0' align='left' underline='true'>" + "Синхронизация с сервером" + "</t><br/><br/>";
			//add _text
			_html = _html + "<t color='#a9b08e' size='1' shadow='0' shadowColor='#312100' align='left'>" + "Подождите секунду, Синхронизация с сервером" + "</t>";
			_html = _html + "<br/><t color='#a9b08e' size='1' shadow='0' shadowColor='#312100' align='left'>" + "Используйте Alt+T чтобы телепортироваться к своей команде" + "</t>";
